...
def _complete_translation_batch_locked(
    ...
    with transaction.atomic():
        ...
        # The text is in its language; now its name, blurb and contents follow.
        transaction.on_commit(lambda: _queue_metadata_translation(edition.id))
...
def _queue_metadata_translation(edition_id) -> None:
    from almonium_book_processor.catalog.metadata_translation import queue_metadata_translation

    try:
        queue_metadata_translation(str(edition_id))
    except ValueError:
        # Recorded on the edition as a review item when the run itself fails;
        # here it only means there is no key or nothing to translate from.
        logger.info("Metadata translation not queued for %s", edition_id, exc_info=True)
