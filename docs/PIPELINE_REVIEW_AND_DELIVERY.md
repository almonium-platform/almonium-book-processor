...
| P1-2 | processor; P1-1 | Implemented: versioned chapter difficulty/summary artifacts, book percentile/distribution and coverage, explicit front/back-matter roles, separate editorial level, stale-result checks and free projection refresh. UI distinguishes pending/partial/stale/failed. See [aggregation rules](CHAPTER_ANALYSIS.md#chapter-and-book-projections). |
| P1-3 | processor; P1-2 | In progress: [EN/DE/UK synthetic fixtures and offline comparison](CALIBRATION_FIXTURES.md) implemented. Authentic excerpts, human ratings and actual model evaluation remain open; no calibrated-accuracy claim yet. First-encounter vocabulary still needs ordered source occurrences. |
...
| P2-2 | processor; P1-2 | [B2 generation](ADAPTATION_PILOT.md) implemented: paid chapter pilots and full-book resumable chunks, keep/adapt decisions, exact source lineage, inherited alignment, atomic completeness and staff review gate; sentence/lexical/difficulty enrichment follows. Current priority: review the full Frankenstein draft and confirm achieved difficulty. No prerequisite modernization or calibration project. Original remains independent; publication stays manual. Add B1 after B2 review. |
